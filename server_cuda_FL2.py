...
class Aggregator:
    def __init__(self, i, args, num_labels, num_train_sam):
        ...
        # lr scheduler
        num_update_steps_per_round = math.ceil(num_train_sam[i] * args.sample_ratio[i])
...
class Aggregator_mom(Aggregator):
    def __init__(self, i, args, num_labels, num_train_sam):
        ...
        self.optimizer = torch.optim.AdamW(optimizer_grouped_parameters, lr=args.learning_rate[i])
        # self.optimizer = SGD(optimizer_grouped_parameters, lr=args.learning_rate)
                ...
                for n, p in aggs[i].model.named_parameters():
                    if n == name:
                        try:
                            ...
                            p = p.to(device)
                            sam_weight[i] = sam_weight[i].to(device)
...
def HeteAgg_mom_FL2(r, logger, args, aggs, dummy_names, leave_one_names, hete_exclude_name, did2_sample_portion,leader_id):
    # print("!!!!!!!!! HETE MOM AGG !!!!!!!!!!! (share sub-large model)")
    with torch.no_grad():
        avg_params = {}
        sam_weight_dict = {}
...
def evaluate(i, r, args, eval_model, accelerator, eval_dataloader, metric, is_regression, logger, best_eval, best_eval_r, loss, processed_datasets, data_collator):
    ...
    if args.task_name in ['mrpc', 'qqp']:
        return eval_metric['accuracy'], eval_metric['f1']
    elif args.task_name == 'cola':
        return eval_metric['matthews_correlation']
    elif args.task_name == 'stsb':
        eval_name = ['spearmanr', 'pearson']
        return eval_metric['spearmanr'], eval_metric['pearson']
    else:

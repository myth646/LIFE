...
def main():
    ...
    for r in tqdm(range(args.rounds)):
        for i in range(args.num_hete_total):
            ...
            if args.task_name in ['mrpc', 'qqp','stsb']:
                result1, result2 = evaluate(i, r, args, aggs[i].model, accelerators[i], eval_dataloader, metric, is_regression, logger, best_eval, best_eval_r, loss, processed_datasets, data_collator)
                eval_metrics_list[0].append(result1)
                eval_metrics_list[1].append(result2)
            else:
                result = evaluate(i, r, args, aggs[i].model, accelerators[i], eval_dataloader, metric, is_regression, logger, best_eval, best_eval_r, loss, processed_datasets, data_collator)
    ...
    for i in range(len(eval_metrics_list)):
        ...
        if not args.manual_distribution:
            ...
            save_to_csv(eval_metrics_list[i], f'{prefix}_{args.task_name}_{eval_name[i]}_noniid({number_str}).csv')   
    # file_paths =[f'output({args.task_name}_each{args.leader_epoch}_{args.strategy}_{args.pick_percentage}).csv',f'output(inclusive_{args.task_name}_each{args.leader_epoch}_{args.strategy}_{args.pick_percentage}).csv']
    # file_paths = [os.path.join('output_diff_datasets', file_name) for file_name in file_paths]
    # printer = CSVDataPrinter(file_paths,'output_seed.csv', args.seed)
    # printer.print_data([0, 1, 2], sum_columns=True)
...
if __name__ == "__main__":
    main()
# python test_cuda_out_all.py --model_name_or_path roberta-base --task_name sst2 --local_cls --local_one --local_pooler --mom_grad --mom_beta 0.2 --log_round 5 --portion 3 2 1 --pick_percentage 0.1 --rounds 100 --strategy same --leader_epoch 5
